...
//   { mark, quantity, type, width_in, height_in, material, unit_price_usd,
...
STEP 2. For each product, extract:
   ...
   g. material — "Aluminum" if the product mentions "Aluminum" or "Aluminium" anywhere (most do). Else infer ("Iron", "Wood"), else "".
   h. unit_price_usd — unit price as a number. If only TOTAL is shown (Format B), divide total by quantity. 0 if not listed.
   i. total_price_usd — total price as a number, no currency symbol or commas. 0 if not listed.
   j. notes — short comma-separated list of important spec callouts: glass type ("Double Tempered Glass", "Low-E"), modifiers ("Argon", "Grid", "Screen", "Tempered"), color ("Matte Black"), shape ("Circle"). Skip boilerplate. Empty string if nothing notable.
  ...
  "items": [
    {
      ...
      "notes": "Matte Black, Double Tempered Glass, Argon, Thermal Break"
    }
  ]
}
...
const ITEM_SCHEMA = {
  ...
  required: ["mark", "quantity", "width_in", "height_in", "type", "operation", "material", "unit_price_usd", "total_price_usd", "notes"],
